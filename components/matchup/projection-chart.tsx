...
export function ProjectionChart({ data, className }: ProjectionChartProps) {
  const latest = data.at(-1)
  const delta = latest ? latest.team - latest.opponent : null

  return (
    <GlassCard tone="default" className={cn("space-y-4", className)}>
      <GlassCard.Header className="space-y-1">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Actual points vs opponent baseline by week.
        </p>
      </GlassCard.Header>
      <GlassCard.Content className="h-[260px]">
        {data.length > 0 ? (
          ...
          <div className="flex h-full items-center justify-center text-sm text-slate-600 dark:text-slate-300">
            Projection data will render once weekly inputs load from the data service.
          </div>
        )}
      </GlassCard.Content>
      {delta !== null && (
        <GlassCard.Footer className="justify-between text-xs uppercase tracking-[0.28em] text-slate-500 dark:text-slate-400">
          <span>Latest delta</span>
          <span>{delta >= 0 ? `+${delta.toFixed(1)} pts` : `${delta.toFixed(1)} pts`}</span>
        </GlassCard.Footer>
      )}
    </GlassCard>
  )
